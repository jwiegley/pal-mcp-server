...
Gemini · OpenAI · Anthropic · Grok · Factory Droid · Azure · Ollama · OpenRouter · DIAL · On-Device Model
...
- **[Factory Droid](https://docs.factory.ai/sdk/python)** - Factory models through Droid SDK (local login or API key)
...
PAL prefers explicit process environment values, then reads the strict user-owned configuration at `$XDG_CONFIG_HOME/pal-mcp/config`, and retains the project-root `.env` as a legacy fallback. The external file is parsed as data—not sourced as shell—must be an owner-only mode-0600 regular file, and accepts only documented provider keys and model allowlists. Factory also supports an explicitly enabled local Droid CLI login. See `.env.example` for the supported names.
...
- **Latest models** - GPT-6 Astra, Gemini 3.8 Flash, Claude Fable 5.1, Grok 4.6, DeepSeek V4 Pro, local Llama
